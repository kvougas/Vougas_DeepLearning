...
localH2O = h2o.init(ip = "localhost", port = 54321, startH2O = TRUE, nthreads=23, max_mem_size="80G")


drugs<-unique(inspect(rhs(rules.sig))$items)
drugs<-gsub("\\{|\\}","",drugs)

result<-dbGetQuery(con,"SELECT Drug,Drug_Status FROM neural_networks_prediction_metrics ORDER BY id DESC LIMIT 1")
...
for (drug in drugs){
  ...
  if (length(predictor.genes.0)==0) {
    query<-paste("INSERT INTO neural_networks_prediction_metrics (Drug,Drug_Status,Predictor_Genes_No) VALUES ('",
  ...
  for (bagging in 1:no.of.rounds){
    ...
    model2 <-
      h2o.deeplearning(x = 2:ncol(temp.train),  # column numbers for predictors
                       ...
                       hidden = c(24), # one layer of 24 nodes
                       epochs = 300, #300 iterations. Einai kata meso oro ta iterations pou xrhsimopoioun.
                       nfolds = 8 #8-fold cross-validation opws sto PLOS ONE.
      ) 
  ...
  drug3<-drug
  drug3<-gsub("\\/","_",drug3)
  filename<-paste("~/Projects/MBA/new_data/Prediction_h2o_nn_quantile_support_filter_GE_NUM/plots/",drug3,".png",sep="")
  ...
  query<-paste("INSERT INTO neural_networks_prediction_metrics (Drug,Drug_Status,Predictor_Genes,Predictor_Genes_No,AUC,Sensitivity,Specificity,PPV,NPV,ACC,FPR) VALUES ('",
               drug2,"','",
               drug.status,"','",
