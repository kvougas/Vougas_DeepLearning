...
con<-dbConnect(SQLite(),"~/Projects/MBA/new_data/ARM_dl.db")
...
result<-dbGetQuery(con,"SELECT Drug,Drug_Status FROM deep_learning_prediction_metrics ORDER BY id DESC LIMIT 1")
...
for (drug in drugs){
  ...
  if (length(predictor.genes.0)==0) {
    query<-paste("INSERT INTO deep_learning_prediction_metrics (Drug,Drug_Status,Predictor_Genes_No) VALUES ('",
...
drug3<-drug
drug3<-gsub("\\/","_",drug3)
filename<-paste("~/Projects/MBA/new_data/Prediction_h2o_dl_quantile_support_filter_GE_NUM/plots/",drug3,".png",sep="")
...
try(temp.fpr<-unlist(performance(pred,"fpr")@y.values)[which.max(unlist(performance(pred,"mat")@y.values))],silent=T)
  ...
  query<-paste("INSERT INTO deep_learning_prediction_metrics (Drug,Drug_Status,Predictor_Genes,Predictor_Genes_No,AUC,Sensitivity,Specificity,PPV,NPV,ACC,FPR) VALUES ('",
               drug2,"','",
               drug.status,"','",
